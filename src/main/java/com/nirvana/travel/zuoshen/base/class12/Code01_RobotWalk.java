...
public class Code01_RobotWalk {
	...
	public static int ways1_optimize(int N, int M, int K, int P) {
		// 参数无效直接返回0
		if (N < 2 || K < 1 || M < 1 || M > N || P < 1 || P > N) {
			return 0;
		}
		//因为cur的取值范围0-N，rest的取值范围0-K
		...
		// 总共N个位置，从M点出发，还剩K步，返回最终能达到P的方法数
		return walk_opt(N, M, K, P, dp);
	}

	public static int walk_opt(int N, int cur, int rest, int P, int[][] dp) {
		...
		if (cur == 1) {
			dp[cur][rest] = walk_opt(N, 2, rest - 1, P, dp);
			return dp[cur][rest];
		}

		if (cur == N) {
			dp[cur][rest] = walk_opt(N, N - 1, rest - 1, P, dp);
			return dp[cur][rest];
		}

		dp[cur][rest] = walk_opt(N, cur + 1, rest - 1, P, dp)
			+ walk_opt(N, cur - 1, rest - 1, P, dp);
		return dp[cur][rest];
	}
